...
namespace Core.ElectricFieldSources
{
    public class Electrode : Positionable<float>, IElectricFieldSource<float> 
    {
        ...
        public Electrode(Vector<float> startPosition,Vector<float> endPosition,int chargesCount,float singleChargeValue,float coulombConstant)
            : base(startPosition)
        {
            ...
            _charges = new CompoundElectricFieldSource<float>(Extensions.CreatePositionsDistribution(Position,EndPosition,chargesCount)
